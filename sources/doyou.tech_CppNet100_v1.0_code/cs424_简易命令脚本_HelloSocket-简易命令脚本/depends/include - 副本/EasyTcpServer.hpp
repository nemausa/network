...
public:
	...
	SOCKET InitSocket()
	{
		...
		if (INVALID_SOCKET == _sock)
		{
            ...
            int flag = 1;
            if (-1 == setsockopt(_sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&flag, sizeof(flag))) {
                CELLLog::Info("setsockopt SO_REUSEADDR fail");
            }
	...
	int Bind(const char* ip, unsigned short port)
	{
		...
		if (ip) {
            ...
            CELLLog::Info("bind ip<%s> success...\n", ip);
		}
		else {
	...
	SOCKET Accept()
	{
		...
		{
			CELLLog::Info("error, accept INVALID_SOCKET...errno<%d>, errMsg<%s>.\n",errno,strerror(errno));
		}
		else
		{
#ifndef _WIN32
            if(cSock >= CELL_MAX_FD)
            {
                close(cSock);
                CELLLog::Error("accept fd<%d> MAX_FD<%d>\n",cSock,CELL_MAX_FD);
...
private:
	...
	{
#ifdef __APPLE__
        CELLLog::Error("FD_SETSIZE<%d> CELL_MAX_FD<%d> <%d><%d><%d>\n",FD_SETSIZE,CELL_MAX_FD,__DARWIN_howmany(__DARWIN_FD_SETSIZE, __DARWIN_NFDBITS),__DARWIN_FD_SETSIZE,__DARWIN_NFDBITS);
#elif __linux__
		CELLLog::Error("FD_SETSIZE<%d> CELL_MAX_FD<%d>\n",FD_SETSIZE,CELL_MAX_FD);
#endif
		while (pThread->isRun())
		{
			time4msg();
			//伯克利套接字 BSD socket
			cell_fd_set fdRead;//描述符（socket） 集合
						  //清理集合
			CELL_FD_ZERO(fdRead);
			//将描述符（socket）加入集合
			CELL_FD_SET(_sock, &fdRead);
			///nfds 是一个整数值 是指fd_set集合中所有描述符(socket)的范围，而不是数量
			///既是所有文件描述符最大值+1 在Windows中这个参数可以写0
			timeval t = { 0, 1};
			int ret = select(_sock + 1, (fd_set*)&fdRead, 0, 0, &t); //
			if (ret < 0)
			{
			...
			//判断描述符（socket）是否在集合中
			if (FD_ISSET(_sock, (fd_set*)&fdRead))
			{
				FD_CLR(_sock, &fdRead);
				Accept();
			}
