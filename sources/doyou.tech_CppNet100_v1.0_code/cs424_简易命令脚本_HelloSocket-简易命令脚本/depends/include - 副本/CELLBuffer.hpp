...
public:
	...
	int write2socket(SOCKET sockfd)
	{
		...
		if (_nLast > 0 && INVALID_SOCKET != sockfd)
		{
            ...
            if(ret <= 0)
			{
				CELLLog::Error("write2socket1:sockfd<%d>nSize<%d>nLast<%d>ret<%d>\n", sockfd,_nSize,_nLast,ret);
				return SOCKET_ERROR;
			}
			if(ret == _nLast)
			{
				...
				CELLLog::Info("write2socket2:sockfd<%d>nSize<%d>nLast<%d>ret<%d>\n", sockfd,_nSize,_nLast,ret);
				_nLast -= ret;
	...
	int read4socket(SOCKET sockfd)
	{
		if (_nSize - _nLast > 0)
		{
			...
			if (nLen <= 0)
			{
				CELLLog::Error("read4socket:sockfd<%d>nSize<%d>nLast<%d>nLen<%d>\n", sockfd,_nSize,_nLast,nLen);
				return SOCKET_ERROR;
			}
			//消息缓冲区的数据尾部位置后移
			_nLast += nLen;
