...
bool easy_epoll_client::on_run(int microseconds) {
    if (is_run()) {
       ...
       if (ret < 0) {
            SPDLOG_LOGGER_INFO(spdlog::get(LOG_NAME), "cell_epoll_client.on_run.wait");
           return false;
       } else if (ret == 0) {
           return true;
       }
           ...
           if (pclient) {
                if (events[i].events & EPOLLIN) {
                    if (SOCKET_ERROR == recv_data()) {
                         SPDLOG_LOGGER_INFO(spdlog::get(LOG_NAME), "socket<{}>on_run.epoll recv_data exit", pclient->sockfd());
                        close();
                        continue;
                    }
                }
                if (events[i].events & EPOLLOUT) {
                    if (SOCKET_ERROR == pclient->send_data_real()) {
                         SPDLOG_LOGGER_INFO(spdlog::get(LOG_NAME), "socket<{}>on_run.epoll sen_data_real exit", pclient->sockfd());
                        close();
                    }
                }
           }
